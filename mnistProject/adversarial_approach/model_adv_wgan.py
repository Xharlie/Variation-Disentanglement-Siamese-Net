...
class VDSN_adv_wgangp(object):
    def __init__(
        ...
        self.discrim_W1 = tf.Variable(tf.random_normal([self.dim_F_V * 2, self.dim_F_V], stddev=0.02), name='discrim_W1')
        self.discrim_b1 = bias_variable([self.dim_F_V], name='dis_b1')
        self.discrim_W2 = tf.Variable(tf.random_normal([self.dim_F_V, 1], stddev=0.02), name='discrim_W2')
        self.discrim_b2 = bias_variable([1], name='dis_b2')
    ...
    def build_model(self, gen_disentangle_weight=1, gen_regularizer_weight=1,
        ...
        Y_right_same = tf.placeholder(tf.float32, [None, self.dim_y])
        Y_right_diff = tf.placeholder(tf.float32, [None, self.dim_y])

        image_real_left = tf.placeholder(tf.float32, [None] + self.image_shape)
        image_real_right_same = tf.placeholder(tf.float32, [None] + self.image_shape)
        image_real_right_diff = tf.placeholder(tf.float32, [None] + self.image_shape)
        #  F_V for variance representation
        #  F_I for identity representation
        F_I_left, F_V_left = self.encoder(image_real_left, reuse=False)
        F_I_right_same, F_V_right_same = self.encoder(image_real_right_same, reuse=True)
        F_I_right_diff, F_V_right_diff = self.encoder(image_real_right_diff, reuse=True)

        h3_right_same = self.generator(F_I_left, F_V_right_same, reuse=False)
        h3_right_diff = self.generator(F_I_left, F_V_right_diff, reuse=True)
        h3_left_same = self.generator(F_I_right_same, F_V_left, reuse=True)
        h3_left_diff = self.generator(F_I_right_diff, F_V_left, reuse=True)

        image_gen_left_same = tf.nn.sigmoid(h3_left_same)
        image_gen_left_diff = tf.nn.sigmoid(h3_left_diff)
        image_gen_right_same = tf.nn.sigmoid(h3_right_same)
        image_gen_right_diff= tf.nn.sigmoid(h3_right_diff)

        errD_same = self.discriminator(tf.concat(axis=1, values=(F_V_left, F_V_right_same)), reuse=False)
        errD_diff = self.discriminator(tf.concat(axis=1, values=(F_V_left, F_V_right_diff)), reuse=True)
        errD = tf.reduce_mean(errD_same) - tf.reduce_mean(errD_diff)
        errG = tf.reduce_mean(errD_diff)

        Y_cla_logits_left = self.classifier(F_I_left, reuse=False)
        Y_cla_logits_right_same = self.classifier(F_I_right_same, reuse=True)
        Y_cla_logits_right_diff = self.classifier(F_I_right_diff, reuse=True)
        ...
        gen_cla_correct_prediction_right_same = tf.equal(tf.argmax(Y_cla_logits_right_same, 1), tf.argmax(Y_right_same, 1))
        gen_cla_accuracy_right_same = tf.reduce_mean(tf.cast(gen_cla_correct_prediction_right_same, tf.float32))

        gen_cla_correct_prediction_right_diff = tf.equal(tf.argmax(Y_cla_logits_right_same, 1), tf.argmax(Y_right_diff, 1))
        gen_cla_accuracy_right_diff = tf.reduce_mean(tf.cast(gen_cla_correct_prediction_right_diff, tf.float32))
        ...
        summary_merge_scalar = tf.summary.merge([summary_gen_recon_cost, summary_gen_disentangle_cost, summary_gen_total_cost,
           summary_gen_cla_accuracy,summary_dis_cost,summary_dis_total_cost,summary_dis_prediction_max,summary_dis_prediction_mean,
           summary_dis_prediction_min,summary_gan_dis_cost,summary_gan_gen_cost,summary_gan_total_cost,])
        ...
        return Y_left, Y_right, image_real_left, image_real_right, gen_recon_cost, gen_disentangle_cost, \
               ...
               gan_gen_cost, gan_dis_cost, gan_total_cost, val_recon_img,summary_merge_scalar,summary_merge_img
    ...
    def discriminator(self, F_Vs, reuse=False):
        # 512 to 512
        h1 = lrelu(tcl.layer_norm(tf.matmul(F_Vs, self.discrim_W1
                                            , 'dis_bn1')))
        # 512 to 10
        h2 = tf.matmul(h1, self.discrim_W2) + self.discrim_b2
        return h2
