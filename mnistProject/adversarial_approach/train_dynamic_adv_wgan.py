...
with tf.Session(config=config) as sess:
    try:
        ...
        while epoch < n_epochs:
            disen_cum = 1
            while disen_cum <= args.dis_series:
                ...
                Xs_right_sameID, Ys_right_sameID = randomPickRight(start_disentangle, end_disentangle, trX,
                                                     trY[index_disentangle[start_disentangle:end_disentangle]],
                                                     indexTable)
                Xs_right_sameID = Xs_right_sameID.reshape([-1, 28, 28, 1]) / 255.

                Xs_right_diffID, Ys_right_diffID = randomPickRight(start_disentangle, end_disentangle, trX,
                                                  trY[index_disentangle[start_disentangle:end_disentangle]], indexTable,
                                                  feature="F_I_F_D_F_V")
                Ys_right_diffID = OneHot(Ys_right_diffID, 10)
                Xs_right_diffID = Xs_right_diffID.reshape([-1, 28, 28, 1]) / 255.
                ...
                if end_disentangle > len(trY):
                    np.random.shuffle(index_disentangle)
                    start_disentangle, end_disentangle = resetIndex()
                # if dis_prediction_val_left[1] <= 0.113:
                #     perfect_series+=1
                # else:
                #     perfect_series = 0
                disen_cum += 1
            ...
            # disen_cum = 1
            # perfect_series=0
            # while disen_cum == 1 or (disen_cum <= args.dis_series and dis_prediction_val_left[0] <= 0.70 and perfect_series <= 200):
            #     Xs_left_disentangle = trX[index_disentangle[start_disentangle:end_disentangle]].reshape(
            #         [-1, 28, 28, 1]) / 255.
            #     Ys_left_disentangle = OneHot(trY[index_disentangle[start_disentangle:end_disentangle]], 10)
            #
